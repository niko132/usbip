...
static void launch_executable(char** args)
{
	info("launching executable... %s %s\n", args[0], args[1]);
        pid_t process = fork();
        if (process == 0) {
                // child
		...
		info("Error while forking: %d\n", errno);
	}

	int status;
        pid_t wait_pid = waitpid(process, &status, 0);
	dbg("waitpid(%lu) status: %d\n", (unsigned long)wait_pid, status);
}
...
static int recv_request_import(int sockfd)
{
	...
	for (int j = 0; j < 3; j++) {
		...
		if (found) {
			...
			if (status == ST_DEV_BUSY) {
				// unbind & rebind port
				char** args1 = (char *[]){"usbip", "unbind", "-b", edev->udev.busid, NULL};
				launch_executable(args1);

				char** args2 = (char *[]){"usbip", "bind", "-b", edev->udev.busid, NULL};
                                launch_executable(args2);
